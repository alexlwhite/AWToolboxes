...
%% THIS IS AN OLD DEPRECATED VERSION. THE NEW computeGazePosAndBlinks can handle bincocular or monocular data. 
%% This version only does monocular and has an old way of detecting changes in velocity. 
% 
...
function [medPos, meanPos, goodTimes, blinkCutTimes, noBlinkIntervals, pDataRemainAfterBlinkCut, pupilMissingTimes] = computeMonocGazePosAndBlinks(time1, time2, edf)
...
if blinkCount>0
    for bci=1:blinkCount
        ...
        if ~isempty(endCutBuffer)
            ...
            %can't cut out more data than we have: 
            if endCutT>length(eyeX), endCutT = length(times); end
        else
