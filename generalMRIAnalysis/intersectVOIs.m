%% function newFile = intersectVOIs(f1,f2,newName,folder)
%
% Combine two BrainVoyager VOIs by taking the intersection of their voxels
% Reads in two file ".voi" text files, and  writes a new file with the same header information, but a list of voxels
% that is the intersection of the two input VOIs. Duplicate voxels are excluded. 
% 
% Inputs: 
...
[i,j,k] = ind2sub(ones(1,3)*voi1.OriginalVMRFramingCubeDim, intersectIs);
newVox = [i j k];
